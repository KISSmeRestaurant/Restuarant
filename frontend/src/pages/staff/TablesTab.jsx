...
const TablesTab = ({ darkMode }) => {
  ...
  const fetchTables = async () => {
    try {
      ...
      const response = await fetch('https://restuarant-sh57.onrender.com/api/tables', {
        headers: {
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      // Update table status
      await fetch(`https://restuarant-sh57.onrender.com/api/tables/${selectedTable._id}/assign-order`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const updateTableStatus = async (tableId, status) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://restuarant-sh57.onrender.com/api/tables/${tableId}/status`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
